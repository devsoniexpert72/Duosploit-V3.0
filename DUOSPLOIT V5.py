...
try:
    ...
    missing_library = str(e).split("No module named ")[1]
    print(f"The following library is not installed: {missing_library}")
    install_libraries = input("Do you want to install the missing libraries? (y/n): ").lower()
    if install_libraries == 'y':
        for library in required_libraries:
            try:
                subprocess.check_call(['pip', 'install', library,])
                print(f'Successfully installed: {library}')
                print('All required libraries installed.RE RUN THIS SCRIPT OR IT WILL BREAK!')
...
if not check_internet_connection():
    print("Connection failed :( try checking your wifi")
    sys.exit()

# Your code below this line will only be executed if there is an internet connection
...
# Create a WebEngineView widget (a web browser)
browser = QWebEngineView()
browser.setWindowTitle("Duosploit | V5.0 (Beta)")

print('Loading Duosploit')


# Define multiple JavaScript codes
print("loading js")
try:
    js_code = """
var element = document.querySelector('._2YmyD');
let checkVisibility = setInterval(function() {
    let element = document.querySelector('.e4VJZ.FQpeZ');
    if (element && window.getComputedStyle(element).display !== 'none') {
        element.style.display = 'none';
    }
}, 0);
// Check if the body element is available
if (document.body) {
 // Create a new div element
 var div = document.createElement('div');
  
 // Set the text content of the div element
 div.textContent = 'Powered by Duolingo Pro BETA';
 ...
 div.style.color = '#ccc';
 div.style.fontFamily = 'Feather, sans-serif'; // Change the font to Feather
 div.style.zIndex = '9999'; // Make sure the label stays on top of other elements
  
 // Append the div element to the body
 document.body.appendChild(div);
} else {
 // The body element is not available, wait for it
 document.addEventListener('DOMContentLoaded', function() {
    // Put the above code inside this function to execute it after the body is loaded
 });
}
// This function will be called every 10 milliseconds
function hideElement() {
    var element = document.querySelector('._2Fc1K');
    if (element) {
        // Set the element's display style to none to hide it
        element.style.display = 'none';
    } else {
        console.log('Element with class _2Fc1K not found.');
    }
}

// Call the hideElement function every 10 milliseconds
setInterval(hideElement, 10);
// This function will be called every 10 milliseconds
function centerElement() {
    ...
    if (element) {
        ...
        // Subtract a certain value to position the bar a little above the center
        var offset = 50; // You can adjust this value as needed
        topPosition -= offset;

        // Set the element's position to relative
        element.style.position = 'relative';

        // Set the element's top position
        element.style.top = topPosition + 'px';
    } else {
        console.log('Element with class _2YmyD not found.');
    }
}

// Call the centerElement function every 10 milliseconds
setInterval(centerElement, 1);
...
async function versionServerStuff(option, status, to, from) {
    ...
    """
except js_error as j:
    print("failed to load javascript :(")

# Inject JavaScript code after the page is fully loaded
def onLoadFinished(ok):
    if ok:
        browser.page().runJavaScript(js_code)  # replace print with your callback if needed

browser.loadFinished.connect(onLoadFinished)

# Load the Duolingo website
browser.load(QUrl('https://www.duolingo.com/learn'))
print("Duosploit is ready to use!")
# Show the browser maximized
...
#https://devs27267.wixsite.com/duosploit
